...
Phiên bản: 1.2.0
Cập nhật: 2026-08-30
Trạng thái: card này mô tả dữ liệu đánh giá và nhánh taxonomy an toàn lịch sử.
MVP hiện dùng model pretrained và không fine-tune. Obstacle public v1.1 có
**276 ảnh, 603 bbox** trước semantic re-audit; v1.2 đang staging và chưa qua
human reviewer. Trạng thái đó không chặn MVP, nhưng dữ liệu chưa được dùng để
huấn luyện, công bố metric cuối hoặc chứng minh an toàn. OCR/VQA vẫn là **0**.
...
Dataset obstacle v1.1 có 207 ảnh ở split lịch sử `train` và 69 ảnh validation,
kế thừa pilot 80 ảnh và bổ sung 196 ảnh đã review từ Open Images V7
validation/test và ADE20K validation. Semantic re-audit v1.2 đã phát hiện ảnh
sai miền, sai/chồng nhãn và
thiếu dense annotation. Vì dự án hiện không fine-tune, split `train` chỉ là tên
lịch sử; nếu dùng lại dữ liệu trong tương lai phải hoàn tất review trước. V1.2
chưa có test set độc lập chứng minh hiệu quả cuối hay an toàn điều hướng.
...
- Development/evaluation: sửa code, chọn ngưỡng, prompt và tiền xử lý cho stack pretrained.
...
- Huấn luyện hoặc chứng nhận một hệ thống điều hướng từ dữ liệu chưa hoàn tất review.
...
Chi tiết tại `split_protocol.md`; validator trong
`../../../src/secondeye/data/protocol.py` bắt lỗi group/scene/video/hash chéo split.
...
- Obstacle v1.1: **276 ảnh/603 bbox; 207 split `train` lịch sử, 69 validation; 15/15 lớp, mọi
  lớp >=20 bbox; manifest SHA-256
  `600d8e9c4be184485cebac1685185ad8d97407ae7d6050d8a2140bf47aa3fb32`**.
  Đây là baseline dữ liệu lịch sử và không được dùng để fine-tune nếu chưa hoàn
  tất semantic re-audit. V1.2 đang staging cho hướng phát triển tương lai, chưa
  có test set 100--150 ảnh đã adjudicate.
